...
`19y` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/19y/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/19y/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee 19y` to
...
python3 ./.trellis/scripts/task.py archive 00-join-19y
```
