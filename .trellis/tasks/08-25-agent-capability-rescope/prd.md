...
### D5：§1.1 一句话定位改为「现场接入网关」

原文每个分句描述的都是被替换掉的能力：「现场**诊断**网关」（诊断已降级为规则库）、「能自己发起**总线实验**的 AI Agent」（正是被替换的场景）、「**链路质量劣化时**自动进入诊断」（触发方式已改为事件主动）、「产出**排查工单**」（输出物已变为候选点表）。

采用版本：

> VelaGuard 是一台运行在 STM32H750B-DK + openvela 上的 **RS485/Modbus 现场接入网关**。它的核心不是采集转发，而是把一条陌生总线变成看得懂的点表：确定性算法扫出从站、解出数据类型与字序，板载 AI Agent 补上算法给不出的语义——这个寄存器是什么物理量、什么单位、正常区间在哪里——并用只读采样验证自己的推断，最后交现场人员在屏幕上确认。

§1.2、§1.4、§7 典型场景与演示脚本均从此句派生，须同步。

### D6：手册与实现解耦，改写为产品文档

用户要求：**手册应当像产品经理给出的设计方案，而不是与开发者耦合。**

这不是新增要求，而是回到手册自己声明的角色。`BOUNDARY.md:126` 的文档分层表已写明 `VelaGuard_*.md` 的职责是 **Product requirements**，是手册自身发生了漂移。

耦合规模：约 **320 / 858 行（37%）** 是实现细节——§16 整章（148 行，开头即写「实现以本章为准」）、§4.1 分层的代码模块名（30 行）、§9 存储路径树（28 行）、§3 硬件实测与软件基线（60 行），以及散落各处的 `nanoMODBUS` / `TIOCSRS485` / `tcdrain()` / `tool_guard.c` / 工具函数签名 / `llm_proxy` realloc。

佐证：**这类内容已在过期**。§3.2「WM8994 树内无驱动」（见 F2）正是典型——写进产品文档的实现细节，代码一动就成了错的，而没人会回头改文档。

处理规则分三类，不一股脑搬走：

| 类别 | 处理 | 对象 |
|---|---|---|
| **搬** | 架构契约，不能删但产品文档不该有 | §16 整章、§9 存储布局 → `docs/velaguard-architecture-decisions.md` |
| **删** | 代码本身就是真相，搬到别处照样过期 | 文件名、函数签名、Kconfig 选项名、代码模块名 |
| **留结论去细节** | 硬件约束是产品边界的成因，须保留因果但去掉实现坐标 | §3 硬件章；细节移入已有的 `docs/velaguard-bringup-known-issues.md` |

同时在手册开头写入防漂移判定原则：

> 本手册回答「做什么」和「为什么」，不回答「怎么实现」。出现文件名、函数签名、Kconfig 选项名或内存地址即为越界。

连带影响：`docs/velaguard-mqtt-contract.md` 开头声明「依据 `VelaGuard_项目手册.md` §8.3–8.6」，§16.2 搬走后须改引用。
...
- R8：手册按 D6 的三类规则解耦为产品文档，且开头写入防漂移判定原则
- R9：解耦须**无损**——搬走的架构决策一条不丢，删掉的只能是「代码即真相」的标识符，硬件约束的**因果**必须保留（例如「运行期不能擦写 QSPI，所以 OTA 必须重启后由片内程序完成」这条因果留，`0x90000000` 与 `FMC Bank6` 去）
...
- 不改 §16 各条决策的**内容**，只改其**存放位置**（推进方案 §2.5 列举的保留决策全部继续有效）
- 不重写 `docs/` 下已有技术文档的正文，只在引用失效处改指向

## Acceptance Criteria

### 场景改写

- [ ] `VelaGuard_项目手册.md` 按 `implement.md` 的章节清单完成改写，§1.1 采用 D5 的定位表述
- [ ] `VelaGuard_推进方案.md` 完成改写，其中 §2 新增「v2 → v3」被推翻决策章节，含 Background 的五条理由
...
- [ ] 手册开头写入防漂移判定原则
- [ ] 新建 `docs/velaguard-architecture-decisions.md`，§16 十节与 §9 存储布局无损迁入
- [ ] §16.6 中属于产品语义的部分（首页显示最高优先级告警、`acknowledge` 不等于 `resolved`、多 active alarm 并存）**留在手册**告警章节，只迁移技术部分
- [ ] 手册全文搜索确认无 `.c` / `.h` 文件名、无函数签名、无 `CONFIG_` 选项名、无十六进制内存地址
- [ ] §3 硬件章保留约束因果，实测细节迁入 `docs/velaguard-bringup-known-issues.md`
- [ ] `docs/velaguard-mqtt-contract.md` 的「依据 §8.3–8.6」引用已更新至新落点
- [ ] 逐条比对迁移前后，确认 §16 的十节决策一条未丢
